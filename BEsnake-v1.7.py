...
def _main_ ():#全游戏主循环
    global Size,elimination,Score_change,Dot_Number,real_highest_score,Score,food_exist,frame_delay,Restart,foodX_permanent,foodY_permanent,initial_length,special_foodX,special_foodY,special_food_exist,special_food_start#全局申明
    turtle.clearscreen()
    turtle.bgcolor(colors["game_background"])#设置游戏背景颜色
    version="BEsnake-v1.7"#版本号兼标题
    ...
    def game_loop():#游戏主循环
        global elimination,frame_delay,Score_change,Dot_Number,real_highest_score,Score,food_exist,foodX_permanent,foodY_permanent,Restart,initial_length,special_foodX,special_foodY,special_food_start,special_food_exist,score_change_number,direction_changed
        ...
        if Score_change==1:
            draw_score()
            Score_change=0
        CoX_old=copy.deepcopy(CoX)
        CoY_old=copy.deepcopy(CoY)
        #得出刷新后的蛇头坐标
        ...
        if score_change_number==5 and special_food_exist==0:
            ...
            score_change_number=0
            special_food_start=time.time()
        screen.ontimer(game_loop, int(frame_delay * 1000))#递归下一次渲染(frame_delay秒后)
        screen.update()#画面刷新 
